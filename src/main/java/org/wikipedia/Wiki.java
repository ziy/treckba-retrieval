 ...
 * greater. Uses the <a href="http://www.mediawiki.org/wiki/API:Main_page">MediaWiki API</a> for
 * most operations. It is recommended that the server runs the latest version of MediaWiki (1.20),
 * otherwise some functions may not work.
 * <p>
 * Extended documentation is available <a
 * href="http://code.google.com/p/wiki-java/wiki/ExtendedDocumentation">here</a>. All wikilinks are
 * relative to the English Wikipedia and all timestamps are in your wiki's time zone.
 * </p>
 * Please file bug reports <a
 * href="http://en.wikipedia.org/w/index.php?title=User_talk:MER-C&action=edit&section=new">here</a>
 * (fast) or at the <a href="http://code.google.com/p/wiki-java/issues/list">Google code bug
 * tracker</a> (slow).
 * 
...
public class Wiki implements Serializable {
  ...
  /**
   * Denotes the namespace for image/file description pages. Has the prefix prefix "File:". Do not
   * create these directly, use upload() instead. (This namespace used to have the prefix "Image:",
   * hence the name.)
   * 
   * @see #MEDIA_NAMESPACE
   * @since 0.25
  ...
  /**
   * Denotes move protection (i.e. only admins can move this page) [move=sysop]. We don't define
   * semi-move protection because only autoconfirmed users can move pages anyway.
   * 
   * @since 0.09
   */
  ...
  /**
   * Denotes move and semi-protection (i.e. autoconfirmed editors can edit the page, but you need to
   * be a sysop to move) [edit=autoconfirmed;move=sysop]. Naturally, this value (4) is equal to
   * SEMI_PROTECTION (1) + MOVE_PROTECTION (3).
   * 
   * @since 0.09
   */
  ...
  /**
   * Creates a new connection to a wiki. WARNING: if the wiki uses a $wgScriptpath other than the
   * default <tt>/w</tt>, you need to call <tt>getScriptPath()</tt> to automatically set it.
   * Alternatively, you can use the constructor below if you know it in advance.
   * 
   * @param domain
  ...
  /**
   * Override/edit this if you need to change the API and human interface url configuration of the
   * wiki. Some example uses:
   * 
   * *Using HTTPS on Wikimedia sites *Server-side cache management (maxage and smaxage API
   * parameters)
   * 
  ...
  /**
   * Sets the editing throttle. Read requests are not throttled or restricted in any way. Default is
   * 10s.
   * 
   * @param throttle
  ...
  /**
   * Detects the $wgScriptpath wiki variable and sets the bot framework up to use it. You need not
   * call this if you know the script path is <tt>/w</tt>. See also [[mw:Manual:$wgScriptpath]].
   * 
   * @throws IOException
  ...
  /**
   * Gets the assertion mode. See [[mw:Extension:Assert Edit]] for what functionality this mimics.
   * Assertion modes are bitmasks.
   * 
  ...
  /**
   * Sets the assertion mode. See [[mw:Extension:Assert Edit]] for what this functionality this
   * mimics. Assertion modes are bitmasks. Default is <tt>ASSERT_NONE</tt>.
   * 
   * @param mode
   *          an assertion mode
  ...
  /**
   * Logs in to the wiki. This method is thread-safe. If the specified username or password is
   * incorrect, the thread blocks for 20 seconds then throws an exception.
   * 
   * @param username
   *          a username
   * @param password
  ...
  public synchronized void login(String username, char[] password) throws IOException,
    ...
    if (line.contains("result=\"Success\"")) {
      ...
      log(Level.INFO, "Successfully logged in as " + username + ", highLimit = " + apihighlimit,
              "login");
    } else {
  ...
  public int getCurrentDatabaseLag() throws IOException {
    ...
    log(Level.INFO, "Current database replication lag is " + lag + " seconds",
   ...
   * 
   * @return a map containing the stats. Use "articles", "pages", "files" "edits", "users" or
   *         "admins" to retrieve the respective value
   * @throws IOException
   *           if a network error occurs
   * @since 0.14
   */
  ...
  /**
   * Gets the version of MediaWiki this wiki runs e.g. 1.20wmf5 (54b4fcb). See also
  ...
  /**
   * Renders the specified wiki markup by passing it to the MediaWiki parser through the API. (Note:
   * this isn't implemented locally because I can't be stuffed porting Parser.php). One use of this
   * method is to emulate the previewing functionality of the MediaWiki software.
   * 
   * @param markup
   *          the markup to parse
  ...
  public String parse(String markup) throws IOException {
    ...
    String response = post(apiUrl + "action=parse",
            "prop=text&text=" + URLEncoder.encode(markup, "UTF-8"), "parse");
  ...
  /**
   * Same as <tt>parse()</tt>, but also strips out unwanted crap. This might be useful to
   * subclasses.
   * 
   * @param in
   *          the string to parse
  ...
  /**
   * Determines the intersection of two lists of pages a and b. Such lists might be generated from
   * the various list methods below. Examples from the English Wikipedia:
   * 
   * <pre>
   * // find all orphaned and unwikified articles
   * String[] articles = Wiki.intersection(
   *         wikipedia.getCategoryMembers(&quot;All orphaned articles&quot;, Wiki.MAIN_NAMESPACE),
  ...
  /**
   * Determines the list of articles that are in a but not b, i.e. a \ b. This is not the same as b
   * \ a. Such lists might be generated from the various lists below. Some examples from the English
   * Wikipedia:
   * 
   * <pre>
   ...
   * String[] images = Wiki.relativeComplement(
   *         wikipedia.getCategoryMembers(&quot;Images lacking a description&quot;),
   ...
   * @param title
   *          the page title
   * @return the name of the talk page corresponding to <tt>title</tt> or "" if we cannot recognise
   *         it
   * @throws IllegalArgumentException
   *           if given title is in a talk namespace or we try to retrieve the talk page of a
  ...
  public HashMap[] getPageInfo(String... pages) throws IOException {
    ...
    String urlstart = query
            + "prop=info&intoken=edit%7Cwatch&inprop=protection%7Cdisplaytitle&titles=";
          ...
          if (exists) {
            ...
            if (z != -1) {
              ...
              if (s.contains("sysop"))
                ...
                info[k].put("protection", s.contains("level=\"sysop\"") ? SEMI_AND_MOVE_PROTECTION
                        : SEMI_PROTECTION);
              }
            } else
              info[k].put("protection", item.contains("type=\"move\"") ? MOVE_PROTECTION
                      : NO_PROTECTION);
          } else {
            ...
            info[k].put("protection", item.contains("type=\"create\"") ? PROTECTED_DELETED_PAGE
                    : NO_PROTECTION);
          }
   ...
   * @return one of the various protection levels (i.e,. NO_PROTECTION, SEMI_PROTECTION,
   *         MOVE_PROTECTION, FULL_PROTECTION, SEMI_AND_MOVE_PROTECTION, PROTECTED_DELETED_PAGE)
   * @throws IOException
   *           if a network error occurs
   * @since 0.10
   */
  ...
  /**
   * Returns the namespace a page is in. No need to override this to add custom namespaces, though
   * you may want to define static fields e.g.
   * <tt>public static final int PORTAL_NAMESPACE = 100;</tt> for the Portal namespace on the
   * English Wikipedia.
   * 
   * @param title
   *          the title of the page
   * @return one of namespace types above, or a number for custom namespaces or ALL_NAMESPACES if we
   ...
   * <tt>namespaceIdentifier(1)</tt> should return "Talk" on en.wp. (This does the exact opposite to
   * <tt>namespace()</tt>). Strings returned are localized.
   * 
   * @param namespace
   *          an integer corresponding to a namespace. If it does not correspond to a namespace, we
   *          assume you mean the main namespace (i.e. return "").
   * @return the identifier of the namespace
   * @throws IOException
   *           if the namespace cache has not been populated, and a network error occurs when
   *           populating it
  ...
  protected void populateNamespaceCache() throws IOException {
    ...
    log(Level.INFO, "Successfully retrieved namespace list (" + (namespaces.size() + 1)
   ...
   * <tt>getImage()</tt> to fetch an image.
   * 
   * @param title
   *          the title of the page.
   ...
   *  </tt> returns the 50 most recent change to the wiki in pretty-print HTML. You should test any
   * use of this method on-wiki through the text <tt>{{Special:Specialpage}}</tt>. Use
   * <tt>getImage()</tt> to fetch an image. Be aware of any transclusion limits, as outlined at
  ...
  /**
   * Edits a page by setting its text to the supplied value. This method is thread safe and blocks
   * for a minimum time as specified by the throttle. The edit will be marked bot if
   * <tt>isMarkBot() == true</tt> and minor if <tt>isMarkMinor() == true</tt>.
   * 
   * @param text
   ...
   *          the title of the page
   * @param summary
   *          the edit summary. See [[Help:Edit summary]]. Summaries longer than 200 characters are
   ...
   *          truncated server-side.
   * @param basetime
   *          the timestamp of the revision on which <tt>text</tt> is based, used to check for edit
   *          conflicts. <tt>null</tt> disables this.
   * @throws IOException
   *           if a network error occurs
   * @throws AccountLockedException
   *           if user is blocked
   ...
   * @see #getPageText
   */
  public void edit(String title, String text, String summary, Calendar basetime)
   ...
   *          truncated server-side.
   * @param section
   *          the section to edit. Use -1 to specify a new section and -2 to disable section
   *          editing.
   * @throws IOException
   *           if a network error occurs
   * @throws AccountLockedException
  ...
  /**
   * Edits a page by setting its text to the supplied value. This method is thread safe and blocks
   * for a minimum time as specified by the throttle.
   * 
   * @param text
   *          the text of the page
   * @param title
   ...
   * @param bot
   *          whether to mark the edit as a bot edit (ignored if one does not have the necessary
   *          permissions)
   * @param section
   *          the section to edit. Use -1 to specify a new section and -2 to disable section
   *          editing.
  ...
  public synchronized void edit(String title, String text, String summary, boolean minor,
    ...
    if (!checkRights(level, false)) {
      ...
      logger.logp(Level.WARNING, "Wiki", "edit()", "[" + getDomain()
              + "] Cannot edit - permission denied.", ex);
      throw ex;
    }
  ...
  /**
   * Creates a new section on the specified page. Leave <tt>subject</tt> as the empty string if you
   * just want to append.
   * 
   * @param title
  ...
  /**
   * Prepends something to the given page. A convenience method for adding maintainance templates,
   * rather than getting and setting the page yourself.
   * 
   * @param title
   *          the title of the page
   * @param stuff
  ...
  /**
   * Gets the list of images used on a particular page. Capped at <tt>max</tt> number of images,
  ...
  public String[] getImagesOnPage(String title) throws IOException {
    ...
    log(Level.INFO, "Successfully retrieved images used on " + title + " (" + temp + " images)",
            "getImagesOnPage");
    return images.toArray(new String[temp]);
  }

  /**
   * Gets the list of categories a particular page is in. Includes hidden categories. Capped at
   * <tt>max</tt> number of categories, there's no reason why there should be more than that.
   * 
   * @param title
   *          a page
  ...
  public String[] getCategories(String title) throws IOException {
    ...
    log(Level.INFO, "Successfully retrieved categories of " + title + " (" + temp + " categories)",
            "getCategories");
   ...
   * Capped at <tt>max</tt> number of templates, there's no reason why there should be more than
   * that.
   * 
   * @param title
   *          a page
  ...
  public String[] getTemplates(String title, int... ns) throws IOException {
    ...
    line = line.substring(line.indexOf("<templates>")); // drop off the first title, which is
                                                        // <tt>title</tt>
    ...
    log(Level.INFO, "Successfully retrieved templates used on " + title + " (" + size
  ...
  public HashMap<String, String> getInterWikiLinks(String title) throws IOException {
    String url = apiUrl + "action=parse&prop=langlinks&llimit=max&page="
  ...
  public String[] getLinksOnPage(String title) throws IOException {
    ...
    int size = links.size();
    log(Level.INFO, "Successfully retrieved links used on " + title + " (" + size + " links)",
            "getLinksOnPage");
    return links.toArray(new String[size]);
  }

  /**
   * Gets the list of sections on a particular page. The returned map pairs the section numbering as
   * in the table of contents with the section title, as in the following example:
   * 
   * 1 => How to nominate 1.1 => Step 1 - Evaluate
   * <p>
  ...
  public LinkedHashMap<String, String> getSectionMap(String page) throws IOException {
    String url = apiUrl + "action=parse&text={{:" + URLEncoder.encode(page, "UTF-8")
            + "}}__TOC__&prop=sections";
  ...
  /**
   * Gets the entire revision history of a page. Be careful when using this method as some pages
   * (such as [[Wikipedia:Administrators' noticeboard/Incidents]] have ~10^6 revisions.
   * 
   * @param title
   *          a page
   ...
   * [[Special:MovePage]]. This method is thread safe and is subject to the throttle.
   * 
   * @param title
   ...
   * @throws UnsupportedOperationException
   *           if the original page is in the Category or Image namespace. MediaWiki does not
   *           support moving of these pages.
   * @throws IOException
   *           if a network error occurs
   * @throws CredentialNotFoundException
   *           if not logged in
  ...
  /**
   * Moves a page. Equivalent to [[Special:MovePage]]. This method is thread safe and is subject to
   * the throttle.
   * 
   * @param title
   *          the title of the page to move
   ...
   *          a reason for the move
   * @param noredirect
   *          don't leave a redirect behind. You need to be a admin to do this, otherwise this
   *          option is ignored.
   * @param movesubpages
   *          move the subpages of this page as well. You need to be an admin to do this, otherwise
  ...
  public synchronized void move(String title, String newTitle, String reason, boolean noredirect,
    ...
    if (user == null || !user.isAllowedTo("move")) {
      ...
      logger.logp(Level.SEVERE, "Wiki", "move()", "[" + domain
              + "] Cannot move - permission denied.", ex);
      throw ex;
    }
    statusCheck();
    ...
    if (!checkRights(level, true)) {
      CredentialException ex = new CredentialException("Permission denied: page is protected.");
      logger.logp(Level.WARNING, "Wiki", "move()", "[" + getDomain()
              + "] Cannot move - permission denied.", ex);
      throw ex;
    }
  ...
  public String export(String title) throws IOException {
    return fetch(
            query + "export&exportnowrap&titles=" + URLEncoder.encode(normalize(title), "UTF-8"),
            "export");
  }
   ...
   * @param oldid
   *          a particular oldid
   * @return the revision corresponding to that oldid. If a particular revision has been deleted,
   *         returns null.
   * @throws IOException
   *           if a network error occurs
   * @since 0.17
  ...
  public Revision getRevision(long oldid) throws IOException {
    ...
    String url = query
            + "prop=revisions&rvprop=ids%7Ctimestamp%7Cuser%7Ccomment%7Cflags%7Csize&revids="
            + oldid;
   ...
   * most recent revisions on that page. If this is not the case, then this method does nothing. See
   * [[mw:Manual:Parameters to index.php#Actions]] (look under rollback) for more information. The
   * edit and reverted edits will be marked as bot if <tt>isMarkBot() == true</tt>.
   * 
   * @param revision
   *          the revision to revert. <tt>revision.isTop()</tt> must be true for the rollback to
   *          succeed
   * @throws IOException
   *           if a network error occurs
   * @throws CredentialNotFoundException
   ...
   * Reverts a series of edits on the same page by the same user quickly provided that they are the
   * most recent revisions on that page. If this is not the case, then this method does nothing. See
   * [[mw:Manual:Parameters to index.php#Actions]] (look under rollback) for more information.
   * 
   * @param revision
   *          the revision to revert. <tt>revision.isTop()</tt> must be true for the rollback to
   *          succeed
   * @param bot
   *          whether to mark this edit and the reverted revisions as bot edits (ignored if we
   *          cannot do this)
   * @param reason
  ...
  public synchronized void rollback(Revision revision, boolean bot, String reason)
    ...
    if (retry)
      log(Level.INFO, "Successfully reverted edits by " + user + " on " + revision.getPage(),
              "rollback");
    retry = true;
  }
   ...
   * resulting from the undo.
   * 
   * @param rev
   *          a revision to undo
   * @param to
   *          the most recent in a range of revisions to undo. Set to null to undo only one
   *          revision.
   * @param reason
  ...
  public synchronized void undo(Revision rev, Revision to, String reason, boolean minor, boolean bot)
    ...
    if (!checkRights(level, false)) {
      CredentialException ex = new CredentialException("Permission denied: page is protected.");
      logger.logp(Level.WARNING, "Wiki", "undo()", "[" + getDomain()
              + "] Cannot edit - permission denied.", ex);
      throw ex;
    }
    String wpEditToken = (String) info.get("token");
  ...
  protected Revision parseRevision(String xml, String title) {
    ...
    Revision revision = new Revision(oldid, timestamp, title, summary, user2, minor, bot, rvnew,
            size);
  ...
  /**
   * Fetches an image file and returns the image data in a <tt>byte[]</tt>. To recover the old
   * behavior (BufferedImage), use
  ...
  /**
   * Fetches a thumbnail of an image file and returns the image data in a <tt>byte[]</tt>. To
  ...
  /**
   * Gets the file metadata for a file. Note that <tt>getImage()</tt> reads directly into a
   * <tt>BufferedImage</tt> object, so you won't be able to get all metadata that way. The keys are:
   * 
   * * size (file size, Integer) * width (Integer) * height (Integer) * mime (MIME type, String) *
  ...
  /**
   * Gets duplicates of this file. Capped at <tt>max</tt> number of duplicates, there's no good
   * reason why there should be more than that. Equivalent to [[Special:FileDuplicateSearch]].
   * 
   * @param file
  ...
  public String[] getDuplicates(String file) throws IOException {
    ...
    log(Level.INFO, "Successfully retrieved duplicates of File:" + file + " (" + size + " files)",
            "getDuplicates");
   ...
   *  title, Wiki.FILE_NAMESPACE)</tt>, as the image may have been deleted. This returns only the
   * live history of an image.
   * 
   * @param title
   *          the title of the image (may contain File)
  ...
  /**
   * Gets an old image revision and returns the image data in a <tt>byte[]</tt>. You will have to do
   * the thumbnailing yourself.
   * 
   * @param entry
   *          the upload log entry that corresponds to the image being uploaded
   * @return the image data that was uploaded, as long as it is still live or null if the image
   *         doesn't exist
   * @throws IOException
   *           if a network error occurs
   * @throws IllegalArgumentException
  ...
  public byte[] getOldImage(LogEntry entry) throws IOException {
    ...
    String url = query
            + "prop=imageinfo&iilimit=max&iiprop=timestamp%7Curl%7Carchivename&titles=File:"
            + title;
   ...
   * uploads into 2^<tt>LOG2_CHUNK_SIZE</tt> byte size chunks. This method is thread safe and
   ...
   *           if not logged in
   * @throws CredentialException
   *           if (page is protected OR file is on a central repository) and we can't upload
   * @throws CredentialExpiredException
   *           if cookies have expired
   * @throws IOException
  ...
  public synchronized void upload(File file, String filename, String contents, String reason)
    ...
    if (user == null || !user.isAllowedTo("upload")) {
      ...
      logger.logp(Level.SEVERE, "Wiki", "upload()", "[" + domain
    ...
    if (!checkRights(level, false)) {
      CredentialException ex = new CredentialException("Permission denied: page is protected.");
      logger.logp(Level.WARNING, "Wiki", "upload()", "[" + getDomain()
              + "] Cannot upload - permission denied.", ex);
      throw ex;
    }
    String wpEditToken = (String) info.get("token");
   ...
   *          the number of users to return
   * @param prefix
   *          list all users with this prefix (overrides start and amount), use "" to not specify
   *          one
   * @return a String[] containing the usernames
   * @throws IOException
   *           if a network error occurs
   ...
   * [[Special:Contributions]]. Be careful when using this method because the user may have a high
   * edit count e.g. <tt>enWiki.contribs("MER-C",
  ...
  /**
   * Gets the contributions by a range of IP v4 addresses. Supported ranges are /8, /16 and /24. Do
   * be careful with this, as calls such as
  ...
  /**
   * Gets the contributions for a user, an IP address or a range of IP addresses. Equivalent to
   * [[Special:Contributions]]. To fetch contribs for an IP range, specify part of an IP address
   * e.g. prefix="127.0." for 127.0.0.0/16; for IPv6 addresses use e.g. prefix="2001:db8:0:0:0:".
   * MediaWiki always fully expands IPv6 addresses and converts all digits A through F to uppercase.
   * (No sanitization is done on IP addresses). Be careful when using <tt>prefix</tt> as it may take
   * too long and/or cause OOM.
   * 
   * @param user
   ...
   *          a list of namespaces to filter by, empty = all namespaces.
   * @param prefix
   *          a prefix of usernames. Overrides <tt>user</tt>. Use "" to not specify one.
   * @return contributions of this user
   * @throws IOException
   *           if a network error occurs
   * @since 0.17
   */
  public Revision[] contribs(String user, String prefix, Calendar end, Calendar start, int... ns)
          throws IOException {
    // prepare the url
    ...
    // clean up
    int size = revisions.size();
    log(Level.INFO, "Successfully retrived contributions for " + (prefix.isEmpty() ? user : prefix)
            + " (" + size + " edits)", "contribs");
    return revisions.toArray(new Revision[size]);
  }
   ...
   * enabled. Messages are sent in plain text (no wiki markup or HTML).
   * 
   * @param user
   ...
   * @since 0.24
   */
  public synchronized void emailUser(User user, String message, String subject, boolean emailme)
          throws IOException, LoginException {
    long start = System.currentTimeMillis();
  ...
  public void watch(String... titles) throws IOException, CredentialNotFoundException {
    /*
     * Ideally, we would have a setRawWatchlist() equivalent in the API, and as such not have to
     * send title.length requests. Then we can do away with watchInternal() and this method will
  ...
  /**
   * Fetches the most recent changes to pages on your watchlist. Data is retrieved from the
   ...
   * textbox in the sidebar. Returns an array of search results, where:
   * 
   * <pre>
   *  results[0] == page name
   ...
   *          a search string
   * @param namespaces
   *          the namespaces to search. If no parameters are passed then the default is
   *          MAIN_NAMESPACE only.
  ...
  public String[][] search(String search, int... namespaces) throws IOException {
    ...
    log(Level.INFO, "Successfully searched for string \"" + search + "\" (" + results.size()
            + " items found)", "search");
  ...
  public String[] imageUsage(String image, int... ns) throws IOException {
    ...
    int size = pages.size();
    log(Level.INFO, "Successfully retrieved usages of File:" + image + " (" + size + " items)",
            "imageUsage");
   ...
   * Alternatively, we can retrive a list of what redirects to a page by setting <tt>redirects</tt>
  ...
  public String[] whatLinksHere(String title, boolean redirects, int... ns) throws IOException {
    ...
    int size = pages.size();
    log(Level.INFO, "Successfully retrieved " + (redirects ? "redirects to " : "links to ") + title
            + " (" + size + " items)", "whatLinksHere");
    return pages.toArray(new String[size]);
  }
  ...
  public String[] whatTranscludesHere(String title, int... ns) throws IOException {
    ...
    int size = pages.size();
    log(Level.INFO, "Successfully retrieved transclusions of " + title + " (" + size + " items)",
            "whatTranscludesHere");
    return pages.toArray(new String[size]);
  }
   ...
   * @param subcat
   *          do you want to return members of sub-categories also? (default: false)
   * @param ns
   *          a list of namespaces to filter by, empty = all namespaces.
   * @return a String[] containing page titles of members of the category
   * @throws IOException
  ...
  public String[] getCategoryMembers(String name, boolean subcat, int... ns) throws IOException {
    ...
    do {
      ...
      for (int x = line.indexOf("title=\""); x >= 0; x = line.indexOf("title=\"", x)) {
        int y = line.indexOf("\" />", x);
        String member = decode(line.substring(x + 7, y));
    ...
    int size = members.size();
    log(Level.INFO,
            "Successfully retrieved contents of Category:" + name + " (" + size + " items)",
            "getCategoryMembers");
    return members.toArray(new String[size]);
  }

  /**
   * Searches the wiki for external links. Equivalent to [[Special:Linksearch]]. Returns two lists,
   * where the first is the list of pages and the second is the list of urls. The index of a page in
   * the first list corresponds to the index of the url on that page in the second list. Wildcards
   * (*) are only permitted at the start of the search string.
   * 
   * @param pattern
   *          the pattern (String) to search for (e.g. example.com, *.example.com)
   * @throws IOException
   *           if a network error occurs
   * @return two lists - index 0 is the list of pages (String), index 1 is the list of urls
   *         (instance of <tt>java.net.URL</tt>)
   * @since 0.06
   */
   ...
   *          a list of namespaces to filter by, empty = all namespaces.
   * @param protocol
   *          one of { http, https, ftp, irc, gopher, telnet, nntp, worldwind, mailto, news, svn,
   *          git, mms } or "" (equivalent to http)
   * @throws IOException
   *           if a network error occurs
   * @return two lists - index 0 is the list of pages (String), index 1 is the list of urls
   *         (instance of <tt>java.net.URL</tt>)
  ...
  public ArrayList[] linksearch(String pattern, String protocol, int... ns) throws IOException {
    ...
    // return value
    log(Level.INFO,
            "Successfully returned instances of external link " + pattern + " (" + ret[0].size()
                    + " links)", "linksearch");
    return ret;
  }
   ...
   * Equivalent to [[Special:Ipblocklist]].
   * 
   * @param user
  ...
  /**
   * Lists currently operating blocks that were made in the specified interval. Equivalent to
   * [[Special:Ipblocklist]].
   * 
   * @param start
   *          the start date
   ...
   * [[Special:Ipblocklist]]. WARNING: cannot tell whether a particular IP is autoblocked as this is
   * non-public data (see also [[bugzilla:12321]] and [[foundation:Privacy policy]]). Don't call
   * this directly, use one of the two above methods instead.
   * 
   * @param user
   *          a particular user that might have been blocked. Use "" to not specify one. May be an
   *          IP (e.g. "127.0.0.1") or a CIDR range (e.g. "127.0.0.0/16") but not an autoblock (e.g.
   *          "#123456").
   * @param start
   ...
   * @since 0.12
   */
  protected LogEntry[] getIPBlockList(String user, Calendar start, Calendar end, int amount)
          throws IOException {
    // quick param check
  ...
  /**
   * Gets the log entries representing actions that were performed on a specific target. Equivalent
   * to [[Special:Log]].
   * 
   * @param target
  ...
  /**
   * Gets all log entries that occurred between the specified dates. WARNING: the start date is the
   * most recent of the dates given, and the order of enumeration is from newest to oldest.
  ...
  /**
   * Gets the last how ever many log entries in the specified log. Equivalent to [[Special:Log]] and
   * [[Special:Newimages]] when <tt>type.equals(UPLOAD_LOG)</tt>.
   * 
   * @param amount
   ...
   * @param action
   *          what action to get (e.g. delete, undelete, etc.), use "" to not specify one
   * @throws IOException
   *           if a network error occurs
   * @throws IllegalArgumentException
   ...
   * target. Equivalent to [[Special:Log]]. WARNING: the start date is the most recent of the dates
   * given, and the order of enumeration is from newest to oldest.
   * 
   * @param start
   *          what timestamp to start. Use null to not specify one.
   * @param end
   *          what timestamp to end. Use null to not specify one.
   * @param amount
   *          the amount of log entries to get. If both start and end are defined, this is ignored.
   *          Use Integer.MAX_VALUE to not specify one.
   * @param log
   *          what log to get (e.g. DELETION_LOG)
   * @param action
   *          what action to get (e.g. delete, undelete, etc.), use "" to not specify one
   ...
   * @since 0.08
   */
  public LogEntry[] getLogEntries(Calendar start, Calendar end, int amount, String log,
          String action, User user, String target, int namespace) throws IOException {
    ...
    do {
      ...
      // parse xml. We need to repeat the test because the XML may contain more than the required
      // amount.
   ...
   * <tt>getIPBlockList()</tt> into LogEntry objects. Override this if you want custom log types.
   * NOTE: if RevisionDelete was used on a log entry, the relevant values will be null.
   * 
   * @param xml
   *          the xml to parse
   * @param caller
  ...
  /**
   * Lists pages with titles containing a certain prefix with a certain protection level and in a
   ...
   * @param level
   *          a protection level. Use NO_PROTECTION to not specify one. WARNING: it is not currently
   *          possible to specify a combination of both semi and move protection
   * @param namespace
   *          a namespace. ALL_NAMESPACES is not suppported, an UnsupportedOperationException will
   *          be thrown.
   ...
   *           if a network error occurs
   */
  public String[] listPages(String prefix, int level, int namespace, int minimum, int maximum)
          throws IOException {
  ...
  /**
   * Fetches data from one of a set of miscellaneous special pages. WARNING: some of these may be
   * *CACHED*, *DISABLED* and/or *LIMITED* on large wikis.
   * 
   * @param page
   *          one of { Ancientpages, BrokenRedirects, Deadendpages, Disambiguations,
   *          DoubleRedirects, Listredirects, Lonelypages, Longpages, Mostcategories, Mostimages,
   *          Mostinterwikis, Mostlinkedcategories, Mostlinkedtemplates, Mostlinked, Mostrevisions,
   *          Fewestrevisions, Shortpages, Uncategorizedcategories, Uncategorizedpages,
   *          Uncategorizedimages, Uncategorizedtemplates, Unusedcategories, Unusedimages,
   *          Wantedcategories, Wantedfiles, Wantedpages, Wantedtemplates, Unwatchedpages,
   *          Unusedtemplates, Withoutinterwiki }. This parameter is *case sensitive*.
  ...
  public String[] queryPage(String page) throws IOException, CredentialNotFoundException {
    ...
    int temp = pages.size();
    log(Level.INFO, "Successfully retrieved [[Special:" + page + "]] (" + temp + " pages)",
            "queryPage");
    return pages.toArray(new String[temp]);
  }

  /**
   * Fetches the <tt>amount</tt> most recently created pages in the main namespace. WARNING: The
   * recent changes table only stores new pages for about a month. It is not possible to retrieve
   * changes before then.
   * 
   * @param amount
  ...
  /**
   * Fetches the <tt>amount</tt> most recently created pages in the main namespace subject to the
   * specified constraints. WARNING: The recent changes table only stores new pages for about a
   * month. It is not possible to retrieve changes before then. Equivalent to [[Special:Newpages]].
   * 
   * @param rcoptions
   *          a bitmask of HIDE_ANON etc that dictate which pages we return (e.g. exclude patrolled
   *          pages => rcoptions = HIDE_PATROLLED).
   * @param amount
  ...
  /**
   * Fetches the <tt>amount</tt> most recently created pages in the specified namespace, subject to
   * the specified constraints. WARNING: The recent changes table only stores new pages for about a
   * month. It is not possible to retrieve changes before then. Equivalent to [[Special:Newpages]].
   * 
   * @param rcoptions
   *          a bitmask of HIDE_ANON etc that dictate which pages we return (e.g. exclude patrolled
  ...
  /**
   * Fetches the <tt>amount</tt> most recent changes in the main namespace. WARNING: The recent
   * changes table only stores new pages for about a month. It is not possible to retrieve changes
  ...
  /**
   * Fetches the <tt>amount</tt> most recent changes in the specified namespace. WARNING: The recent
   * changes table only stores new pages for about a month. It is not possible to retrieve changes
   * before then. Equivalent to [[Special:Recentchanges]].
   * <p>
   * Note: Log entries in recent changes have a revid of 0!
  ...
  /**
   * Fetches the <tt>amount</tt> most recent changes in the specified namespace subject to the
   * specified constraints. WARNING: The recent changes table only stores new pages for about a
   ...
   * @since 0.23
   */
  protected Revision[] recentChanges(int amount, int rcoptions, boolean newpages, int... ns)
          throws IOException {
    StringBuilder url = new StringBuilder(query);
    ...
    do {
      ...
      for (int i = line.indexOf("<rc "); i >= 0 && revisions.size() < amount; i = line.indexOf(
              "<rc ", i)) {
        int j = line.indexOf("/>", i);
    ...
    log(Level.INFO, "Successfully retrieved recent changes (" + temp + " revisions)",
            "recentChanges");
   ...
   * 
   * <p>
   * For WMF wikis, see <a href="http://meta.wikimedia.org/wiki/Interwiki_map"> the interwiki
  ...
  /**
   * Fetches all pages that use interwiki links with a certain <tt>prefix</tt> and <tt>title</tt>.
   * <tt>prefix</tt> refers to the wiki being linked to and <tt>title</tt> refers to the page on
   * said wiki being linked to. In wiki syntax, this is [[prefix:title]]. This does not resolve
   * nested prefixes, e.g. [[wikt:fr:Test]].
   * 
   * <p>
   * Example: If [[Test]] and [[Spam]] both contain the interwiki link [[testwiki:Blah]] then
   * <tt>getInterWikiBacklinks("testwiki", "Blah");
   ...
   * @param title
   *          the title of the page on the other wiki to search for (optional, use "|" to not
  ...
  public String[][] getInterWikiBacklinks(String prefix, String title) throws IOException {
    ...
    do {
      ...
      if (line.contains("iwblcontinue")) {
        ...
        iwblcontinue = null;
      // parse
      // form: <iw pageid="24163544" ns="0" title="Elisabeth_of_Wroclaw" iwprefix="pl"
      // iwtitle="Main_Page" />
      for (int x = line.indexOf("<iw "); x >= 0; x = line.indexOf("<iw ", x)) {
        ...
        links.add(new String[] { line.substring(a, b),
            line.substring(c, d) + ':' + line.substring(e, f) });
        x = f;
      }
    } while (iwblcontinue != null);
    log(Level.INFO, "Successfully retrieved interwiki backlinks (" + links.size() + " interwikis)",
  ...
  public class User implements Cloneable {
    ...
    /**
     * Gets various properties of this user. Groups and rights are cached for the current logged in
     * user. Returns:
     * 
     * <pre>
     *  {
    ...
    /**
     * Returns true if the user is allowed to perform the specified action. Uses the rights cache.
     * Read [[Special:Listgrouprights]] before using this!
     * 
     * @param right
     *          a specific action
    ...
    public LogEntry[] blockLog() throws IOException {
      return getLogEntries(null, null, Integer.MAX_VALUE, BLOCK_LOG, "", null, "User:" + username,
              USER_NAMESPACE);
    }
    ...
    /**
     * Fetches the internal edit count for this user, which includes all live edits and deleted
     * edits after (I think) January 2007. If you want to count live edits only, use the slower
  ...
  public class LogEntry implements Comparable<LogEntry> {
     ...
     * @param timestamp
     *          the local time when the action was performed. We will convert this back into a
     *          Calendar.
     * @param details
     *          the details of the action (e.g. the new title of the page after a move was
     *          performed).
     * @since 0.08
     */
    protected LogEntry(String type, String action, String reason, User user, String target,
            String timestamp, Object details) {
      this.type = type;
      this.action = action;
      this.reason = reason;
      this.user = user;
     ...
     * returns null if the reason was RevisionDeleted.
     * 
    ...
    /**
     * Gets the user object representing who performed the action. WARNING: returns null if the user
     * was RevisionDeleted.
     * 
     ...
     * <tr>
     * <td>PROTECTION_LOG
     * <td>action == "protect" or "modify" => the protection level (int, -2 if unrecognized), action
     * == "move_prot" => the old title, else null
     * <tr>
     ...
     * timestamp (e.g. 20071216160302). To tell these apart, feed it into <tt>Long.parseLong()</tt>
  ...
  public class Revision implements Comparable<Revision> {
     ...
     * 
     * @param revid
     *          the id of the revision (this is a long since {{NUMBEROFEDITS}} on en.wikipedia.org
     *          is now (January 2012) ~25% of <tt>Integer.MAX_VALUE</tt>
     * @param timestamp
     ...
     *          the edit summary
     * @param user
     *          the user making this revision (may be anonymous, if not use
     *          <tt>User.getUsername()</tt>)
     * @param minor
    ...
    public String getRenderedText() throws IOException {
      ...
      log(Level.INFO, "Successfully retrieved rendered text of revision " + revid,
              "Revision.getRenderedText");
      return decode(temp);
    }
    ...
    /**
     * Returns a HTML rendered diff table between this revision and the given text. Useful for
     * emulating the "show changes" functionality. See the table at the <a
     * href="http://en.wikipedia.org/w/index.php?diff=343490272">example</a>.
     * 
     * @param text
     *          some wikitext
     ...
     * 
     * @param oldid
     *          the oldid of a revision on the same page. NEXT_REVISION, PREVIOUS_REVISION and
     *          CURRENT_REVISION can be used here for obvious effect.
     * @return the difference between this and the other revision
     * @throws IOException
     *           if a network error occurs
     * @since 0.26
     ...
     * WARNING: Will return false for all revisions prior to 2007 (I think?) -- this is a MediaWiki
     * problem.<br>
     * WARNING: Returning true does not imply this is the bottommost revision on the page due to
     * histmerges.<br>
    ...
    /**
     * Returns the edit summary for this revision. WARNING: returns null if the summary was
     * RevisionDeleted.
     * 
    ...
    /**
     * Returns the user or anon who created this revision. You should pass this (if not an IP) to
     * <tt>getUser(String)</tt> to obtain a User object. WARNING: returns null if the user was
     * RevisionDeleted.
     * 
    ...
    /**
     * Sets the <tt>rcid</tt> of this revision, used for patrolling. This parameter is optional.
    ...
    /**
     * Gets the rollback token for this revision. Can be null, and often for good reasons: cannot
     * rollback or not top revision.
     * 
  ...
  /**
   * A generic URL content fetcher. This is only useful for GET requests, which is almost everything
   * that doesn't modify the wiki. Might be useful for subclasses.
   * 
   * Here we also check the database lag and wait if it exceeds <tt>maxlag</tt>, see <a
    ...
    if (lag > maxlag) {
      try {
        synchronized (this) {
          ...
          log(Level.WARNING, "Current database lag " + lag + " s exceeds " + maxlag
                  + " s, waiting " + time + " s.", caller);
   ...
   *          the url to post to
   * @param params
   *          the POST parameters. Supported types: UTF-8 text, byte[]. Text and parameter names
   ...
   * @since 0.27
   */
  protected String multipartPost(String url, Map<String, ?> params, String caller)
          throws IOException {
    // set up the POST
    logurl(url, caller);
    URLConnection connection = new URL(url).openConnection();
  ...
  /**
   * Creates a Calendar object with the current time. Wikimedia wikis use UTC, override this if your
   * wiki is in another timezone.
   * 
   * @return see above
   * @since 0.26
   */
  ...
  /**
   * Turns a timestamp of the format yyyymmddhhmmss into a Calendar object. Might be useful for
   * subclasses.
   * 
   * @param timestamp
  ...
  /**
   * Converts a timestamp of the form used by the API (yyyy-mm-ddThh:mm:ssZ) to the form
