...
public class MultiSearcherRetrievalStrategist extends
        AbstractSimpleQueryGenerationLuceneRetrievalStrategist {
  ...
  protected List<RetrievalResult> retrieveDocuments(List<String> queries) {
    ...
    try {
      ...
      for (ScoreDoc hit : hits) {
        results.add(new RetrievalResult(searcher.doc(hit.doc).get("stream-id"), hit.score, queries
                .get(0)));
      }
    } catch (IOException e) {
      e.printStackTrace();
